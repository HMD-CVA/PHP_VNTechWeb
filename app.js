...
// Session middleware
app.use(session({
    secret: process.env.SESSION_SECRET || 'vntechwebsecret2024',
    resave: false,
...
mongoApiRouter.get('/regions', async (req, res) => {
    try {
        const regions = await DataModel.RegionModel.find({ trang_thai: 1 }).sort({ ma_vung: 1 }).lean();
        res.json(regions);
    } catch (error) {
...
mongoApiRouter.get('/thuonghieu', async (req, res) => {
    try {
        const brands = await DataModel.BrandModel.find({}).sort({ ngay_tao: -1 }).lean();
        res.json(brands);
    } catch (error) {
...
mongoApiRouter.get('/danhmuc', async (req, res) => {
    try {
        const categories = await DataModel.CategoryModel.find({}).sort({ thu_tu: 1, ngay_tao: -1 }).lean();
        res.json(categories);
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
...
mongoApiRouter.get('/sanpham', async (req, res) => {
    try {
        const { vung_id } = req.query;
        const filter = {};
        if (vung_id) filter.site_created = vung_id;

        const products = await DataModel.ProductModel.find(filter)
            .populate('thuong_hieu_id', 'ten_thuong_hieu logo_url')
            .populate('danh_muc_id', 'ten_danh_muc')
            .sort({ ngay_tao: -1 })
            .lean();

        const ids = products.map((p) => p._id);
        const variants = await DataModel.ProductVariantModel.find({ san_pham_id: { $in: ids } }).lean();
        const byProduct = new Map();
        variants.forEach((variant) => {
            const key = String(variant.san_pham_id);
        ...
        const payload = products.map((product) => {
            ...
            const active = list.filter((v) => v.trang_thai === 1);
            const giaBan = active.map((v) => v.gia_ban || 0);
            const giaNiemYet = active.map((v) => v.gia_niem_yet || 0);

            return {
                ...product,
                id: product._id,
                ten_thuong_hieu: product.thuong_hieu_id?.ten_thuong_hieu || '',
                ten_danh_muc: product.danh_muc_id?.ten_danh_muc || '',
                brand_logo: product.thuong_hieu_id?.logo_url || '',
                variants: list,
                so_bien_the: list.length,
                so_bien_the_active: active.length,
                so_bien_the_vung: active.length,
                gia_ban_min: giaBan.length ? Math.min(...giaBan) : product.gia_ban || 0,
                gia_ban_max: giaBan.length ? Math.max(...giaBan) : product.gia_ban || 0,
                gia_niem_yet_min: giaNiemYet.length ? Math.min(...giaNiemYet) : product.gia_niem_yet || 0,
                gia_niem_yet_max: giaNiemYet.length ? Math.max(...giaNiemYet) : product.gia_niem_yet || 0,
                tong_ton_kho: active.reduce((sum, v) => sum + (v.so_luong_ton_kho || 0), 0),
                tong_luot_ban: active.reduce((sum, v) => sum + (v.luot_ban || 0), 0),
                chi_tiet: {}
            };
        });
        ...
        res.status(500).json({ success: false, message: 'Đã xảy ra lỗi khi lấy dữ liệu sản phẩm', error: error.message });
    }
});

mongoApiRouter.get('/variants', async (req, res) => {
    try {
        const { site_origin } = req.query;
        const productFilter = site_origin ? { site_created: site_origin } : {};
        const products = await DataModel.ProductModel.find(productFilter).select('_id ten_san_pham link_anh_dai_dien').lean();
        const ids = products.map((p) => p._id);

        const query = { trang_thai: 1 };
        if (site_origin) query.san_pham_id = { $in: ids };

        const variants = await DataModel.ProductVariantModel.find(query).sort({ ngay_tao: -1 }).lean();
        const productMap = new Map(products.map((p) => [String(p._id), p]));

        const data = variants.map((variant) => {
            const product = productMap.get(String(variant.san_pham_id));
            return {
                ...variant,
                ten_san_pham: product?.ten_san_pham || '',
                san_pham_anh: product?.link_anh_dai_dien || ''
            };
        });

        res.json({ success: true, data, total: data.length });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Lỗi server khi lấy danh sách variants', error: error.message });
    }
});

mongoApiRouter.get('/products/:productId/variants', async (req, res) => {
    try {
        const variants = await DataModel.ProductVariantModel.find({ san_pham_id: req.params.productId }).sort({ ngay_tao: -1 }).lean();
...
mongoApiRouter.get('/flashsales', async (req, res) => {
    try {
        const flashSales = await DataModel.FlashSaleModel.find({}).sort({ ngay_tao: -1 }).lean();
...
mongoApiRouter.get('/flashsales/:id/items', async (req, res) => {
    try {
        const items = await DataModel.FlashSaleItemModel.find({ flash_sale_id: req.params.id }).sort({ thu_tu: 1, ngay_tao: -1 }).lean();
...
mongoApiRouter.get('/provinces', async (req, res) => {
    try {
        const { vung_id } = req.query;
        const filter = {};
        if (vung_id) filter.vung_id = vung_id;
        const provinces = await DataModel.ProvinceModel.find(filter).sort({ ten_tinh: 1 }).lean();
...
mongoApiRouter.get('/wards', async (req, res) => {
    try {
        const { tinh_thanh_id } = req.query;
        const filter = {};
        if (tinh_thanh_id) filter.tinh_thanh_id = tinh_thanh_id;
        const wards = await DataModel.WardModel.find(filter).sort({ ten_phuong_xa: 1 }).lean();
...
mongoApiRouter.get('/users', async (req, res) => {
    try {
        const users = await DataModel.UserModel.find({}).sort({ ngay_dang_ky: -1 }).lean();
...
mongoApiRouter.get('/user-addresses/:userId', async (req, res) => {
    try {
        const addresses = await DataModel.UserAddressModel.find({ user_id: req.params.userId, trang_thai: { $ne: 0 } })
            .populate('phuong_xa_id')
            .sort({ is_default: -1, ngay_tao: -1 })
            .lean();
...
mongoApiRouter.get('/inventory', async (req, res) => {
    try {
        const inventory = await DataModel.InventoryModel.find({})
            .populate('variant_id')
            .populate('kho_id')
            .sort({ ngay_tao: -1 })
            .lean();
...
mongoApiRouter.get('/warehouses', async (req, res) => {
    try {
        const warehouses = await DataModel.WarehouseModel.find({}).sort({ ngay_tao: -1 }).lean();
...
mongoApiRouter.get('/warehouses/by-region/:regionId', async (req, res) => {
    try {
        const warehouses = await DataModel.WarehouseModel.find({ vung_id: req.params.regionId, trang_thai: { $ne: 0 } }).lean();
        res.json({ success: true, data: warehouses });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
...
mongoApiRouter.get('/shipping-methods', async (req, res) => {
    try {
        const shippingMethods = await DataModel.ShippingMethodModel.find({}).sort({ chi_phi_co_ban: 1 }).lean();
        res.json({ success: true, data: shippingMethods });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Lỗi khi lấy danh sách phương thức vận chuyển' });
    }
});

mongoApiRouter.get('/shipping-methods/by-region/:regionId', async (req, res) => {
    try {
        const rows = await DataModel.ShippingMethodRegionModel.find({ region_id: req.params.regionId, trang_thai: 1 })
            .populate('shipping_method_id')
            .lean();
        const data = rows.map((row) => ({
            ...row,
            ten_phuong_thuc: row.shipping_method_id?.ten_phuong_thuc || '',
            mo_ta: row.shipping_method_id?.mo_ta || '',
            chi_phi_co_ban: row.shipping_method_id?.chi_phi_co_ban || 0
        }));
...
mongoApiRouter.get('/shipping-method-regions', async (req, res) => {
    try {
        const rows = await DataModel.ShippingMethodRegionModel.find({}).populate('shipping_method_id').lean();
...
mongoApiRouter.get('/vouchers', async (req, res) => {
    try {
        const { page = 1, limit = 10, loai_giam_gia, search, trang_thai } = req.query;
        const filter = {};
        if (loai_giam_gia) filter.loai_giam_gia = loai_giam_gia;
        if (search) {
            filter.$or = [
                { ma_voucher: { $regex: search, $options: 'i' } },
                { ten_voucher: { $regex: search, $options: 'i' } }
            ];
        }

        const now = new Date();
        if (trang_thai === 'active') {
            filter.ngay_bat_dau = { $lte: now };
            filter.ngay_ket_thuc = { $gte: now };
        } else if (trang_thai === 'expired') {
            filter.ngay_ket_thuc = { $lt: now };
        }

        const vouchers = await DataModel.VoucherModel.find(filter).sort({ ngay_tao: -1 }).lean();
        const pageNum = parseInt(page, 10);
        const limitNum = parseInt(limit, 10);
        const start = (pageNum - 1) * limitNum;
        const rows = vouchers.slice(start, start + limitNum);

        res.json({
            success: true,
            vouchers: rows,
            currentPage: pageNum,
            totalPages: Math.ceil(vouchers.length / limitNum),
            total: vouchers.length
        });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Lỗi khi lấy danh sách voucher: ' + error.message });
    }
});

mongoApiRouter.get('/vouchers/:id/products', async (req, res) => {
    try {
        const links = await DataModel.VoucherProductModel.find({ voucher_id: req.params.id }).populate('san_pham_id').lean();
...
mongoApiRouter.post('/vouchers/validate', async (req, res) => {
    try {
        const { ma_voucher, tong_tien_hang = 0 } = req.body;
        const now = new Date();
        const voucher = await DataModel.VoucherModel.findOne({ ma_voucher }).lean();
        if (!voucher) {
        ...
        if (voucher.ngay_bat_dau && now < new Date(voucher.ngay_bat_dau)) {
            return res.status(400).json({ success: false, message: 'Voucher chưa đến thời gian sử dụng' });
        }
        ...
        if ((tong_tien_hang || 0) < (voucher.dieu_kien_ap_dung || 0)) {
            return res.status(400).json({ success: false, message: 'Đơn hàng chưa đủ điều kiện áp dụng voucher' });
        }

        let giaTriGiam = 0;
        if (voucher.loai_giam_gia === 'phan_tram') {
            giaTriGiam = Math.floor((tong_tien_hang * voucher.gia_tri_giam) / 100);
            if (voucher.gia_tri_giam_toi_da) {
                giaTriGiam = Math.min(giaTriGiam, voucher.gia_tri_giam_toi_da);
            }
        } else {
            giaTriGiam = voucher.gia_tri_giam;
        }

        return res.json({ success: true, data: { voucher, gia_tri_giam: giaTriGiam } });
    } catch (error) {
        return res.status(500).json({ success: false, message: error.message });
    }
});

mongoApiRouter.get('/orders', async (req, res) => {
    try {
        const { userId } = req.query;
        const filter = {};
        if (userId) filter.nguoi_dung_id = userId;

        const orders = await DataModel.OrderModel.find(filter)
            .populate('dia_chi_giao_hang_id')
            .populate('voucher_id')
            .sort({ ngay_tao: -1 })
            .lean();
...
mongoApiRouter.get('/orders/:orderId', async (req, res) => {
    try {
        const order = await DataModel.OrderModel.findById(req.params.orderId)
            .populate('dia_chi_giao_hang_id')
            .populate('voucher_id')
            .lean();
        if (!order) {
        ...
        const details = await DataModel.OrderDetailModel.find({ don_hang_id: order._id })
            .populate('variant_id')
            .populate('warehouse_id')
            .populate('flash_sale_item_id')
            .lean();
...
mongoApiRouter.put('/orders/:orderId/cancel', async (req, res) => {
    try {
        const order = await DataModel.OrderModel.findByIdAndUpdate(
            req.params.orderId,
            { trang_thai: 'huy' },
            { new: true }
        ).lean();
...
mongoApiRouter.get('/donhang', async (req, res) => {
    try {
        const orders = await DataModel.OrderModel.find({}).sort({ ngay_tao: -1 }).lean();
        res.json({ success: true, data: orders });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
...
mongoApiRouter.get('/donhang/:id', async (req, res) => {
    try {
        const order = await DataModel.OrderModel.findById(req.params.id).lean();
        if (!order) return res.status(404).json({ success: false, message: 'Không tìm thấy đơn hàng' });
        const details = await DataModel.OrderDetailModel.find({ don_hang_id: order._id }).lean();
        res.json({ success: true, data: { order, details } });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
...
mongoApiRouter.put('/donhang/:id/status', async (req, res) => {
    try {
        const { trang_thai, ghi_chu, nguoi_thao_tac } = req.body;
        const oldOrder = await DataModel.OrderModel.findById(req.params.id).lean();
        if (!oldOrder) return res.status(404).json({ success: false, message: 'Không tìm thấy đơn hàng' });

        const updated = await DataModel.OrderModel.findByIdAndUpdate(req.params.id, { trang_thai }, { new: true }).lean();
        await DataModel.OrderStatusHistoryModel.create({
            don_hang_id: updated._id,
            trang_thai_cu: oldOrder.trang_thai,
            trang_thai_moi: trang_thai,
            ghi_chu: ghi_chu || '',
            nguoi_thao_tac: toObjectId(nguoi_thao_tac) || oldOrder.nguoi_dung_id
        });
...
// Global middleware để load data cho header (categories)
app.use(async (req, res, next) => {
    try {
        // Load categories cho tất cả các view - dùng .lean() để trả về plain objects
        const categories = await DataModel.CategoryModel.find({}).lean();

        // Map _id thành id để Handlebars có thể truy cập
        res.locals.categories = categories.map(c => ({ ...c, id: c._id }));
...
app.get('/', async (req, res) => {
    try {
        const sanphams = await DataModel.ProductModel.find({}).lean();
        const brands = await DataModel.BrandModel.find({}).lean();
        const provinces = await DataModel.ProvinceModel.find({}).lean();
        const allFlashSales = await DataModel.FlashSaleModel.find({}).sort({ ngay_bat_dau: 1 }).lean();
        const now = new Date();
        ...
        for (const flashSale of allFlashSales) {
            ...
            const computedStatus = endDate < now ? 'da_ket_thuc' : (startDate <= now ? 'dang_dien_ra' : 'sap_dien_ra');

            const items = await DataModel.FlashSaleItemModel.find({ flash_sale_id: flashSale._id, trang_thai: { $ne: 'dung_ban' } }).lean();
            const enrichedItems = [];

            for (const item of items) {
                const variant = await DataModel.ProductVariantModel.findById(item.variant_id).lean();
                if (!variant) continue;
                const product = await DataModel.ProductModel.findById(variant.san_pham_id).lean();
                if (!product) continue;
                ...
                enrichedItems.push({
                    ...
                    flash_sale_item_id: item._id,
                    ten_san_pham: product.ten_san_pham,
                    ten_variant: `${product.ten_san_pham} - ${variant.ten_hien_thi || 'Mặc định'}`,
                    variant_name: variant.ten_hien_thi || 'Mặc định',
                    link_anh: variant.anh_dai_dien || product.link_anh_dai_dien || '/image/default-product.png',
                    sku: variant.ma_sku,
                    gia_goc: item.gia_goc,
                    gia_flash_sale: item.gia_flash_sale,
                    gia_ban_formatted: new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.gia_flash_sale),
                    gia_goc_formatted: new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.gia_goc),
                    phan_tram_giam,
                    so_luong_ton,
                    da_ban,
                    con_lai,
                    da_ban_percent,
                    ton_kho_variant: variant.so_luong_ton_kho || 0,
            ...
            flashSaleEvents.push({
                info: {
                    ...
                    is_active: computedStatus === 'dang_dien_ra',
                    is_upcoming: computedStatus === 'sap_dien_ra',
                    is_ended: computedStatus === 'da_ket_thuc'
                },
                items: enrichedItems
            });
        }
        ...
        flashSaleEvents.forEach(event => {
            event.items.forEach(item => {
                if (item.product_id) {
                    flashSaleProductIds.add(String(item.product_id));
                }
            });
        });

        const formattedProducts = sanphams.map(product => {
            const hasFlashSale = flashSaleProductIds.has(String(product._id));
        ...
        brands.forEach(brand => {
            brandProductGroups[String(brand._id)] = {
                brandInfo: brand,
                products: []
            };
        });
        ...
        formattedProducts.forEach(product => {
            const brandId = String(product.thuong_hieu_id || '');
            if (brandId && brandProductGroups[brandId]) {
                brandProductGroups[brandId].products.push(product);
            }
        });
...
app.get('/admin', async (req, res) => {
    try {
        ...
        const dbInfo = {
            database: 'MongoDB',
            vung_id: req.session.user.vung_id || 'N/A',
    ...
    try {
        res.json({
            ...
            sessionInfo: {
                ...
                vai_tro: req.session.user?.vai_tro || 'N/A'
            }
        });
    } catch (err) {
...
app.get('/product/:id', async (req, res) => {
    try {
        ...
        const product = await DataModel.ProductModel.findById(productId).lean();
        ...
        try {
            const flashSaleItems = await DataModel.FlashSaleItemModel.findActiveByProductId(productId);
        ...
        const inventory = await DataModel.InventoryModel.find({variant_id: productId}).lean();

        // ✅ Load product_variants với thông tin flash sale
        const productVariants = await DataModel.ProductVariantModel.find({san_pham_id: productId}).lean();
        ...
        const variantsWithFlashSale = await Promise.all(productVariants.map(async (variant) => {
            try {
                ...
                const flashSaleItem = await DataModel.FlashSaleItemModel.findOne({variant_id: variant._id, trang_thai: {$in: ['dang_ban']}}).lean();
                ...
                console.error('Error loading flash sale for variant:', variant._id, err);
                return {
                    ...variant,
                    id: variant._id,
                    isFlashSale: false,
...
// Route GET /admin/sanpham - Hiển thị trang quản lý sản phẩm - MongoDB version
app.get('/admin/sanpham', async (req, res) => {
    try {
        // Lấy danh sách categories từ MongoDB với .lean()
        const categories = await DataModel.CategoryModel.find({ trang_thai: 1 })
            .sort({ thu_tu: 1 })
            .select('_id ten_danh_muc slug')
            .lean();

        // Lấy danh sách brands từ MongoDB với .lean()
        const brands = await DataModel.BrandModel.find({})
            .select('_id ten_thuong_hieu slug logo_url')
            .lean();

        // Lấy danh sách regions từ MongoDB với .lean()
        const regions = await DataModel.RegionModel.find({ trang_thai: 1 })
            .sort({ ma_vung: 1 })
            .select('ma_vung ten_vung')
            .lean();

        // Chuyển _id thành id cho tương thích với frontend
        const categoriesWithId = categories.map(c => ({ ...c, id: c._id }));
        const brandsWithId = brands.map(b => ({ ...b, id: b._id }));
...
app.post('/api/products/:productId/variants', async (req, res) => {
    try {
        ...
        try {
            await DataModel.InventoryModel.syncInventoryForVariant(
                newVariant.id,
            ...
            if (productResult.recordset.length > 0 && productResult.recordset[0].mongo_detail_id) {
                ...
                if (mongoDoc) {
                    ...
                    if (calculated_price !== null && calculated_price !== undefined) {
                        ...
                        await DataModel.ProductModel.update(updateProductData, san_pham_id);
...
app.post('/api/sanpham', async (req, res) => {
    try {
        ...
        // Logic thêm sản phẩm
        const newProduct = await DataModel.ProductModel.create(productData);
...
app.put('/api/sanpham/:id', async (req, res) => {
    try {
        const productId = req.params.id;
        const updateData = req.body;
        await DataModel.ProductModel.update(updateData, productId);
...
app.delete('/api/sanpham/:id', async (req, res) => {
    try {
        ...
        const product = await DataModel.ProductModel.findById(productId);
        if (!product) {
            return res.status(404).json({
                success: false,
                message: 'Không tìm thấy sản phẩm'
        ...
        // Xóa sản phẩm từ SQL
        const result = await DataModel.ProductModel.destroy({
            where: { id: productId }
        });
...
app.get('/api/thuonghieu/:id', async (req, res) => {
    try {
        ...
        const brand = await DataModel.BrandModel.findById(brandId);
...
app.post('/api/thuonghieu', async (req, res) => {
    try {
        ...
        const existingBrand = await DataModel.BrandModel.findOne({ where: { slug } });
        ...
        const newBrand = await DataModel.BrandModel.create({
...
app.put('/api/thuonghieu/:id', async (req, res) => {
    try {
        ...
        const existingBrand = await DataModel.BrandModel.findById(brandId);
        if (!existingBrand) {
            return res.status(404).json({
                success: false,
                message: 'Không tìm thấy thương hiệu'
        ...
        if (brandData.ten_thuong_hieu.trim() !== existingBrand.ten_thuong_hieu) {
            ...
            // Kiểm tra slug trùng lặp
            const allBrands = await DataModel.BrandModel.find({});
        ...
        const updatedBrand = await DataModel.BrandModel.update(brandId, updateData);
...
app.delete('/api/thuonghieu/:id', async (req, res) => {
    try {
        ...
        const result = await DataModel.BrandModel.destroy({
            where: { id: brandId }
        });
...
app.get('/api/danhmuc', async (req, res) => {
    try {
        ...
        const categories = await DataModel.CategoryModel.findAll({
...
app.get('/api/danhmuc/:id', async (req, res) => {
    try {
        ...
        const category = await DataModel.CategoryModel.findById(categoryId);
...
app.post('/api/danhmuc', async (req, res) => {
    try {
        ...
        // Kiểm tra slug trùng lặp
        const existingCategory = await DataModel.CategoryModel.findOne({ where: { slug } });
        if (existingCategory) {
            return res.status(400).json({
                success: false,
                message: 'Slug đã tồn tại, vui lòng chọn tên khác'
        ...
        const newCategory = await DataModel.CategoryModel.create({
...
app.put('/api/danhmuc/:id', async (req, res) => {
    try {
        ...
        const existingCategory = await DataModel.CategoryModel.findById(categoryId);
        if (!existingCategory) {
            return res.status(404).json({
                success: false,
                message: 'Không tìm thấy danh mục'
        ...
        if (categoryData.ten_danh_muc.trim() !== existingCategory.ten_danh_muc) {
            ...
            const allCategories = await DataModel.CategoryModel.find({});
        ...
        const updatedCategory = await DataModel.CategoryModel.update(categoryId, updateData);
...
app.delete('/api/danhmuc/:id', async (req, res) => {
    try {
        ...
        const result = await DataModel.CategoryModel.destroy({
            where: { id: categoryId }
        });
...
app.post('/api/sanpham', async (req, res) => {
    try {
        ...
        // Kiểm tra SKU trùng
        const existingProduct = await DataModel.ProductModel.findOne({
        ...
        const newProduct = await DataModel.ProductModel.create({
...
app.put('/api/sanpham/:id', async (req, res) => {
    try {
        ...
        const existingProduct = await DataModel.ProductModel.findById(productId);
        if (!existingProduct) {
            return res.status(404).json({
                success: false,
                message: 'Không tìm thấy sản phẩm'
        ...
        if (productData.ma_sku && productData.ma_sku !== existingProduct.ma_sku) {
            const duplicateProduct = await DataModel.ProductModel.findOne({
                where: { ma_sku: productData.ma_sku }
            });
        ...
        const updatedProduct = await DataModel.ProductModel.update(productId, updateData);
...
app.put('/api/sanpham/:id/status', async (req, res) => {
    try {
        ...
        const updatedProduct = await DataModel.ProductModel.update(productId, {
...
app.post('/api/mongo/sanpham', async (req, res) => {
    try {
        ...
        if (savedDetail.calculated_price !== null && sql_product_id) {
            try {
                const sqlProduct = await DataModel.ProductModel.findById(sql_product_id);
                if (sqlProduct) {
                    ...
                    await DataModel.ProductModel.update(updatePriceData, sql_product_id);
                }
            } catch (sqlError) {
...
app.put('/api/mongo/sanpham/:id', async (req, res) => {
    try {
        ...
        if (updatedDetail.calculated_price !== null && updatedDetail.sql_product_id) {
            try {
                const sqlProduct = await DataModel.ProductModel.findById(updatedDetail.sql_product_id);
                if (sqlProduct) {
                    const updatePriceData = {
                    ...
                    await DataModel.ProductModel.update(updatePriceData, updatedDetail.sql_product_id);
                }
            } catch (sqlError) {
                console.error('⚠️ Failed to update SQL price:', sqlError);
            }
...
app.get('/admin/flashsale', async (req, res) => {
    try {
        // Fetch regions from MongoDB with .lean()
        const regions = await DataModel.RegionModel.find({ trang_thai: 1 })
            .sort({ ten_vung: 1 })
            .select('ma_vung ten_vung')
            .lean();
...
app.get('/api/products-with-variants', async (req, res) => {
    try {
        ...
        const products = await DataModel.ProductModel.find({});
...
app.get('/api/flashsales', async (req, res) => {
    try {
        ...
        const flashSales = await DataModel.FlashSaleModel.findAll(req.dbPool, filters);
...
app.get('/api/flashsales/:id', async (req, res) => {
    try {
        const flashSale = await DataModel.FlashSaleModel.findById(req.params.id);
...
app.post('/api/flashsales', async (req, res) => {
    try {
        ...
        const newFlashSale = await DataModel.FlashSaleModel.create(flashSaleData);
        ...
        const updatedFlashSale = await DataModel.FlashSaleModel.update(newFlashSale.id, {
...
app.put('/api/flashsales/:id', async (req, res) => {
    try {
        ...
        const updatedFlashSale = await DataModel.FlashSaleModel.update(req.params.id, updateData);
...
app.delete('/api/flashsales/:id', async (req, res) => {
    try {
        // Xóa từ SQL
        await DataModel.FlashSaleModel.destroy(req.params.id);
...
app.get('/api/flashsales/:flashSaleId/items/:itemId', async (req, res) => {
    try {
        const item = await DataModel.FlashSaleItemModel.findById(req.params.itemId);
...
app.post('/api/flashsales/:id/items', async (req, res) => {
    try {
        ...
        const newItem = await DataModel.FlashSaleItemModel.create(itemData);
...
app.put('/api/flashsales/:flashSaleId/items/:itemId', async (req, res) => {
    try {
        ...
        const updatedItem = await DataModel.FlashSaleItemModel.update(req.params.itemId, updateData);
...
app.delete('/api/flashsales/:flashSaleId/items/:itemId', async (req, res) => {
    try {
        await DataModel.FlashSaleItemModel.destroy(req.params.itemId);
...
app.get('/api/provinces', async (req, res) => {
    try {
        ...
        const provinces = await DataModel.ProvinceModel.findAll(req.dbPool, filters);
...
app.get('/api/provinces/:id', async (req, res) => {
    try {
        const province = await DataModel.ProvinceModel.findById(req.params.id);
...
app.post('/api/provinces', async (req, res) => {
    try {
        ...
        const newProvince = await DataModel.ProvinceModel.create(provinceData);
...
app.put('/api/provinces/:id', async (req, res) => {
    try {
        ...
        const updated = await DataModel.ProvinceModel.update(req.params.id, updateData);
...
app.delete('/api/provinces/:id', async (req, res) => {
    try {
        const deleted = await DataModel.ProvinceModel.delete(req.params.id);
...
app.get('/api/wards', async (req, res) => {
    try {
        ...
        const wards = await DataModel.WardModel.findAll(req.dbPool, filters);
...
app.get('/api/wards/:id', async (req, res) => {
    try {
        const ward = await DataModel.WardModel.findById(req.params.id);
...
app.post('/api/wards', async (req, res) => {
    try {
        ...
        const newWard = await DataModel.WardModel.create(wardData);
...
app.put('/api/wards/:id', async (req, res) => {
    try {
        ...
        const updated = await DataModel.WardModel.update(req.params.id, updateData);
...
app.delete('/api/wards/:id', async (req, res) => {
    try {
        const deleted = await DataModel.WardModel.delete(req.params.id);
...
app.get('/admin/nguoidung', async (req, res) => {
    try {
        // Lấy danh sách users từ MongoDB với .lean()
        const users = await DataModel.UserModel.find({})
            .sort({ ngay_dang_ky: -1 })
            .lean();
...
app.get('/api/users', async (req, res) => {
    try {
        ...
        let users = await DataModel.UserModel.findAll(req.dbPool, filters);
...
app.post('/api/users', async (req, res) => {
    try {
        ...
        const existingUser = await DataModel.UserModel.findOne({email: email});
        if (existingUser) {
            return res.status(409).json({
                success: false,
        ...
        // Create user in SQL
        const newUser = await DataModel.UserModel.create({
            name,
            email,
            phone: phone || null,
            vung_id: vung_id || 'bac',
        ...
        try {
            ...
            await DataModel.UserModel.update(newUser.id, {
                ...newUser,
...
app.get('/api/users/:id/profile', async (req, res) => {
    try {
        ...
        const user = await DataModel.UserModel.findById(id);
...
app.put('/api/users/:id', async (req, res) => {
    try {
        ...
        const existingUser = await DataModel.UserModel.findById(id);
        if (!existingUser) {
            return res.status(404).json({
                success: false,
                message: 'Không tìm thấy người dùng'
        ...
        if (email && email !== existingUser.email) {
            const userWithSameEmail = await DataModel.UserModel.findOne({email: email});
        ...
        const updatedUser = await DataModel.UserModel.update(id, updateData);
...
app.delete('/api/users/:id', async (req, res) => {
    try {
        ...
        const deleted = await DataModel.UserModel.delete(id);
...
app.put('/api/users/:id/status', async (req, res) => {
    try {
        ...
        const updatedUser = await DataModel.UserModel.updateStatus(id, newStatus);
...
app.get('/admin/inventory', async (req, res) => {
    try {
        // Load warehouses from MongoDB with .lean()
        const warehouses = await DataModel.WarehouseModel.find({})
            .select('_id ten_kho dia_chi vung_id')
            .lean();

        // Load products from MongoDB with .lean()
        const products = await DataModel.ProductModel.find({ trang_thai: 1 })
            .select('_id ma_san_pham ten_san_pham')
            .sort({ ten_san_pham: 1 })
            .lean();

        // Add id field for compatibility
        const warehousesWithId = warehouses.map(w => ({ ...w, id: w._id }));
        const productsWithId = products.map(p => ({ ...p, id: p._id }));

        res.render('inventory', {
            layout: 'AdminMain',
            title: 'Quản lý Tồn kho',
            warehouses: warehousesWithId,
...
app.get('/api/inventory', async (req, res) => {
    try {
        ...
        const inventory = await DataModel.InventoryModel.findAll(req.dbPool);
...
app.get('/api/inventory/:id', async (req, res) => {
    try {
        ...
        const inventoryItem = await DataModel.InventoryModel.findById(id);
...
app.post('/api/inventory', async (req, res) => {
    try {
        ...
        const newInventory = await DataModel.InventoryModel.create(inventoryData);
...
app.put('/api/inventory/:id', async (req, res) => {
    try {
        ...
        const existingInventory = await DataModel.InventoryModel.findById(id);
        ...
        const updatedInventory = await DataModel.InventoryModel.update(id, inventoryData);
...
app.put('/api/inventory/:id/adjust', async (req, res) => {
    try {
        ...
        const existingInventory = await DataModel.InventoryModel.findById(id, req.dbPool);
        if (!existingInventory) {
            return res.status(404).json({
                success: false,
                message: 'Không tìm thấy tồn kho'
        ...
        const updatedInventory = await DataModel.InventoryModel.update(id, {
...
app.post('/api/inventory/upsert', async (req, res) => {
    try {
        ...
        const existingInventory = await DataModel.InventoryModel.findByVariantId(bien_the_san_pham_id);
        ...
        if (existingInventory) {
            ...
            result = await DataModel.InventoryModel.update(existingInventory.id, updateData);
        } else {
            // Get variant to find warehouse for its region
            const variant = await DataModel.ProductVariantModel.findById(bien_the_san_pham_id);
            ...
            const warehouses = await DataModel.WarehouseModel.findByRegion(variant.site_origin);
            ...
            result = await DataModel.InventoryModel.create(newInventoryData);
        }
...
app.delete('/api/inventory/:id', async (req, res) => {
    try {
        ...
        await DataModel.InventoryModel.delete(id);
...
app.get('/api/flashsales', async (req, res) => {
    try {
        ...
        const flashSales = await DataModel.FlashSaleModel.findAll(filters);
        res.json(flashSales);
    } catch (error) {
...
app.get('/api/flash-sales/:id/items', async (req, res) => {
    try {
        ...
        const items = await DataModel.FlashSaleItemModel.findByFlashSaleId(req.params.id);
...
app.post('/api/flash-sales', async (req, res) => {
    try {
        ...
        const flashSale = await DataModel.FlashSaleModel.create(flashSaleData);

        // Add variants to flash_sale_items
        if (variants && variants.length > 0) {
            for (const variant of variants) {
                await DataModel.FlashSaleItemModel.create({
...
app.put('/api/flash-sales/:id', async (req, res) => {
    try {
        ...
        const flashSale = await DataModel.FlashSaleModel.update(req.params.id, flashSaleData);

        // Update variants - smart update (delete/update/insert only what changed)
        if (variants) {
            // Lấy danh sách items hiện tại
            const currentItems = await DataModel.FlashSaleItemModel.findByFlashSaleId(req.params.id);
            ...
            for (const item of itemsToDelete) {
                await DataModel.FlashSaleItemModel.destroy(item.id);
            }
            ...
            for (const variant of variants) {
                ...
                if (variant.id && currentItemIds.includes(variant.id)) {
                    ...
                    await DataModel.FlashSaleItemModel.update(variant.id, itemData);
                } else {
                    // Thêm mới item
                    const newItem = await DataModel.FlashSaleItemModel.create(itemData);
                }
...
app.delete('/api/flash-sales/:id', async (req, res) => {
    try {
        ...
        await DataModel.FlashSaleItemModel.deleteByFlashSaleId(req.params.id);
    ...
    try {
        ...
        const enrichedItems = await Promise.all(items.map(async (item) => {
            try {
                ...
                const product = await DataModel.ProductModel.findById(foundDoc.sql_product_id);
...
app.post('/api/flashsales/:id/items', async (req, res) => {
    try {
        ...
        const item = await DataModel.FlashSaleItemModel.create(itemData);
...
app.put('/api/flashsales/:id', async (req, res) => {
    try {
        const flashSale = await DataModel.FlashSaleModel.update(req.params.id, req.body);
        res.json({ success: true, data: flashSale });
    } catch (error) {
        console.error('Flash Sale PUT Error:', error);
...
app.get('/api/warehouses', async (req, res) => {
    try {
        ...
        const warehouses = await DataModel.WarehouseModel.findAll(req.dbPool);
...
app.get('/api/warehouses/:id', async (req, res) => {
    try {
        ...
        const warehouse = await DataModel.WarehouseModel.findById(id);
...
app.post('/api/warehouses', async (req, res) => {
    try {
        ...
        const newWarehouse = await DataModel.WarehouseModel.create(warehouseData);
...
app.put('/api/warehouses/:id', async (req, res) => {
    try {
        ...
        const existingWarehouse = await DataModel.WarehouseModel.findById(id);
        ...
        const updatedWarehouse = await DataModel.WarehouseModel.update(id, warehouseData);
...
app.delete('/api/warehouses/:id', async (req, res) => {
    try {
        ...
        const inventoryCount = await DataModel.InventoryModel.countByWarehouse(id);
        if (inventoryCount > 0) {
            return res.status(400).json({
                success: false,
        ...
        await DataModel.WarehouseModel.delete(id);
